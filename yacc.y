...
void add_func(table *table, func *func);
void add_var(table *table, var *var);
void free_table(table *env);
void free_all_table(table *env);
...
%type <node> function parameter_list parameter_list_no_empty type_list param func_body void_body declarations
...
%type <node> nested_statements_with_return nested_statements stmt code_block declaration_no_function conditions loops multi_assign update func_call expression_list return
...
initial:
	program							
    {  startSemantics($1); /* printTree($1,0);*/  to3AC($1); freeTree($1);}
	;
...
program:
    ...
    { $$ = mknode("CODE",1, $1); } 
	|program cmd					
    { $$ = combineNodes("CODE",$1, mknode("CODE",1,$2)); }
	;
...
function:
	 VALTYPE ID '(' parameter_list ')' '{' func_body '}'					
	 		            	{ $$ = mknode("FUNCTION",4,mknode($2,0),$4,mknode("TYPE",1,mknode($1,0)),$7); }
	|
	 VOID ID '(' parameter_list ')' '{' void_body '}'	
        { $$ = mknode("FUNCTION",4,mknode($2,0),$4,mknode("TYPE",1,mknode("VOID",0)),$7);}
	; 
...
parameter_list:
	parameter_list_no_empty						
    	{ $$ = $1;}
	|epsilon									
    	{ $$ = mknode("ARGS",1,mknode("NONE",0)); }
	;

parameter_list_no_empty:
	type_list ';' parameter_list_no_empty			
    { $$ = combineNodes("ARGS",$1,$3); }
	|type_list									
    	{ $$ = $1;}
	;
...
type_list:	
	VALTYPE	param									
    { $2->token = strdup($1); $$ = mknode("ARGS",1, $2); }
	;

param:
	ID ',' param									
         { $$ = combineNodes("ARGS", mknode($1,0), $3); }
	|ID												
         { $$ = mknode("ARGS",1,mknode($1,0)); }
	;

func_body:
	declarations nested_statements_with_return	
        { $$ = combineNodes("BODY",$1,$2); }
	|nested_statements_with_return				
        { free($1->token); $1->token =strdup("BODY"); $$=$1; }
	;

void_body:
	declarations nested_statements			
        	{ $$ = combineNodes("BODY",$1,$2); }
    |declarations							
    	{ free($1->token); $1->token=strdup("BODY"); $$=$1; }
	|nested_statements					
    		{ free($1->token); $1->token =strdup("BODY"); $$=$1; }
	|epsilon						
    			{ $$ = mknode("BODY",1,mknode("NONE",0)); }
	;

declarations:
	declaration declarations			
    		{ $$ = combineNodes("BODY",mknode("BODY",1,$1),$2); }
	|declaration						
    		{ $$ = mknode("BODY",1,$1); }
...
primitive_declaration:
	VAR VALTYPE declaration_parameters ';'		
    			{ $$ = combineNodes("VAR", mknode("VAR",1,mknode($2,0)), $3); }
	;

declaration_parameters:
	ID ',' declaration_parameters					
    		{ $$ = combineNodes("VAR", mknode("VAR",1,mknode($1,0)), $3); }
	|primitive_assign ',' declaration_parameters	
    		{ $$ = combineNodes("VAR", mknode("ASS",1,$1) ,$3); }
	|ID												
    		{ $$ = mknode($1,0); }
	|primitive_assign								
    		{ $$ = mknode("ASS",1,$1); }
...
string_declaration:
	string_parameters ',' string_declaration		
    		{ $$ = combineNodes("STRING", $1, $3);}
	|string_parameters								
    		{ $$ = $1;}
    ;
...
string_parameters:
	ID '[' expression ']'						
    			{ $$ = mknode("STRING",1,mknode($1,1,$3)); }
	|ID '[' expression ']' ASS STRINGVAL		
    			{ $$ = mknode("STRING",1,mknode($5,2,mknode($1,1,$3),mknode($6,0))); }
	;

/*-------------------------------------------Statments---------------------------------*/

nested_statements_with_return:
	stmt nested_statements_with_return				
    		{ $$ = combineNodes("Statments", mknode("stmt",1,$1),$2);}
	|return ';'									
    			{ $$ = mknode("stmt",1,$1);}
	;
...
primitive_assign:
    ID ASS expression							
    			{ $$ = mknode($2,2, mknode($1,0), $3); }
    ;
index_assign:
	ID '[' expression ']' ASS expression		
    			{ $$ = mknode($5,2,mknode($1,1,$3),$6); }
	;	
string_assign:
	ID ASS STRINGVAL								
    		{ $$ = mknode($2,2,mknode($1,0),mknode($3,0)); }
	;
pointer_assign:
	MUL ID ASS expression							
    		{ $$ = mknode($3, 2 , mknode("POINTER", 1, mknode($2,0)), $4); }
	;
/*----------------------------------------Code Block------------------------------*/

code_block:
	'{' declaration_no_function nested_statements '}'			
    		{ $$ = combineNodes("BLOCK",$2, $3); }
	|'{' declaration_no_function '}'						
    			{ $$ = $2; }
	|'{' nested_statements '}'									
    		{free($2->token); $2-> token = strdup("BLOCK"); $$ = $2; }
	|'{' epsilon '}'											
    		{ $$ = mknode("BLOCK",1,mknode("NONE",0)); }
	;
declaration_no_function:
	declaration declaration_no_function							
    		{ $$ = combineNodes("BLOCK", mknode("BLOCK",1,$1), $2);}
	|declaration										
    				{ $$ = mknode("BLOCK", 1, $1);}
	;
...
conditions:
	IF '(' expression ')' stmt %prec LOWER_THEN_ELSE	
    	{ $$ = mknode("IF", 2, $3, $5); }
...
multi_assign:
	primitive_assign ',' multi_assign			
    			{ $$ = combineNodes("ass", mknode("ass",1,$1),$3); }
	|primitive_assign						
    				{ $$ = mknode("ass",1,$1); }
	;
update:
	ID PLUS PLUS							
    				{ $$ = mknode("UPDATE",1,mknode("=",2, mknode($1,0), mknode("+", 2, mknode($1,0), mknode("int",1, mknode("1",0))))); }
	| ID MINUS MINUS					
    					{ $$ = mknode("UPDATE",1,mknode("=",2, mknode($1,0), mknode("-", 2, mknode($1,0), mknode("int",1,mknode("1",0))))); }		
	| multi_assign						
    					{ free($1->token); $1-> token = strdup("UPDATE"); $$ = $1; }
	;
...
func_call:
	ID '(' expression_list ')'						
    		{ $$ = combineNodes("FUNC-CALL", mknode($1,0), $3); }
	;
expression_list:
	expression ',' expression_list					
    		{ $$ = combineNodes("paramters",mknode("param",1,$1), $3);}
	|expression									
    			{ $$ = mknode("paramters",1,$1); }
	|epsilon									
    			{ $$ = mknode("NONE",0); }
	;
	...
	for (int i = 1; i < tree->numOfSubNodes; i++) {
		...
		if (tree->subNodes[i]->subNodes == NULL) {
			...
			if (checkDupVar(stable, tree->subNodes[i]->token)) 
				add_var(stable, mkvar(tree->subNodes[i]->token, type)); //add var to symbol table
		...
		/* variable with assign */
			...
			if (checkDupVar(stable, tree->subNodes[i]->subNodes[0]->token))
				add_var(stable, mkvar(tree->subNodes[i]->subNodes[0]->token, type));
			/* var was declared already */
			else {
				printf("Error: Duplicate variable name - %s\n", tree->subNodes[i]->subNodes[0]->token);
				quitProgram(fullTree, stable);
...
void insertString(table *stable, node *tree, node* fullTree) {
	...
	for (int i = 0; i < tree->numOfSubNodes; i++) {
		...
		/* variable with no assign */
			...
			/* check string wasnt declared already */
			if (checkDupVar(stable, tree->subNodes[i]->token))
				add_var(stable, mkvar(tree->subNodes[i]->token, type));
			/* string was declared already */
			else {
				printf("Error: Duplicate variable name - %s\n", tree->subNodes[i]->token);
				quitProgram(fullTree, stable);
	...
	for (int i = 0; i < tree->numOfSubNodes; i++) {
		...
		for (int j = 0; j < tree->subNodes[i]->numOfSubNodes; j++) {
			...
			if (checkDupVar(stable, tree->subNodes[i]->subNodes[j]->token))
				add_var(stable, mkvar(tree->subNodes[i]->subNodes[j]->token, type));
			/* var was declared already */
			else {
...
void add_func(table *stable, func *funcAdd) {
	func **functions;
...
void add_var(table *table, var *varAdd) {
...
void checkTree(node *subTree , table *env, node *tree) {
	...
	*/
		...
		/* create new environment for function code and send function void_body for semantic check after recursive return 
	...
	*/
	...
	if (env->returnType <= 6 && env->returnType >= 0) {
		...
		if (evalExp(subTree->subNodes[0], env, tree) != env->returnType) {
            if(evalExp(subTree->subNodes[0], env, tree)==3 &&env->returnType==2||(evalExp(subTree->subNodes[0], env, tree)==2 && env->returnType==3))
                 {
                    return true;
                 }
			return false;
	...
	*/
		...
		for(int i=1; i<tree->numOfSubNodes; i++){
			...
			if(i == 1 && flag == 0){
				...
				int size=strlen(e1->var);
				/*printf("strlen of e1->var is- %d\n",size);*/
			}
			/* else realloc */
			else{
		...
		sizeOfPop=numOfPush*4;
		
		sprintf(l,"%d",sizeOfPop);
